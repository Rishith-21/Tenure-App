...
const ProfileUpdateMoreScreen = ({navigation, route}: any) => {
  ...
  useEffect(() => {
    loadProfileSocial().then(links => {
      const items: SocialLinkItem[] = SOCIAL_PLATFORMS.filter(
        ...
        label: p.label,
        icon: p.icon,
        url: links[p.id as ProfileSocialPlatformId]!,
      }));
  ...
  const handleSave = async () => {
    const stored: ProfileSocialLinks = {};
    for (const link of socialLinks) {
      stored[link.platformId as ProfileSocialPlatformId] = link.url;
    }
    await saveProfileSocial(stored);
    showAlert({
      title: 'Updated',
  ...
  return (
    <>
      ...
      <NativeDateTimePicker
        ...
        onConfirm={picked => {
          setEndTime(picked);
          if (picked.getTime() <= startTime.getTime()) {
